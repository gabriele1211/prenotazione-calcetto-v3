...
function showBox(id, text, type = "warning") {
  const box = $(id); box.textContent = text; box.className = `message show ${type}`;
}
...
async function saveSettings() {
  const dal = $("chiusura-dal").value || null, al = $("chiusura-al").value || null;
...
async function loadBookings() {
  const body = $("prenotazioni-body"); body.innerHTML = '<tr><td colspan="9">Caricamento...</td></tr>';
  let query = db.from("prenotazioni").select(`id,campo_id,nome_cliente,telefono,documento_numero,documento_data_rilascio,documento_rilasciato_da,data,ora_inizio,ora_fine,stato,note,campi(nome)`).order("data", { ascending: true }).order("ora_inizio", { ascending: true });
  ...
  $("admin-message").textContent = bookings.length ? `${bookings.length} prenotazioni visualizzate.` : "Nessuna prenotazione.";
  body.innerHTML = "";
  ...
  bookings.forEach(item => {
    ...
    cells.forEach((text, i) => { const td = document.createElement("td"); if (i === 3) { const strong = document.createElement("strong"); strong.textContent = text; td.appendChild(strong); } else td.textContent = text; tr.appendChild(td); });
    ...
    if (item.note) { const badge = document.createElement("span"); badge.className = "request-badge"; badge.textContent = "📌 Richiesta"; const p = document.createElement("p"); p.className = "request-text"; p.textContent = item.note; noteTd.append(badge, p); } else noteTd.textContent = "—";
    tr.appendChild(noteTd);

    const statusTd = document.createElement("td"); const status = document.createElement("span"); status.className = `status ${item.stato}`; status.textContent = item.stato; statusTd.appendChild(status); tr.appendChild(statusTd);
    const actionTd = document.createElement("td"); const button = document.createElement("button"); button.className = "mini-button"; button.type = "button"; button.dataset.id = item.id; button.dataset.status = item.stato === "annullata" ? "confermata" : "annullata"; button.textContent = item.stato === "annullata" ? "Ripristina" : "Annulla"; actionTd.appendChild(button); tr.appendChild(actionTd);
    body.appendChild(tr);
  });
...
async function checkSession() {
  const { data } = await db.auth.getSession(); const logged = Boolean(data.session);
  $("login-box").classList.toggle("hidden", logged); $("dashboard").classList.toggle("hidden", !logged);
...
$("filtro-da").value = today.toISOString().slice(0, 10); $("filtro-a").value = plus30.toISOString().slice(0, 10);
checkSession();
